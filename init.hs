...
main ={- (<*) (putStrLn "hello") $ const (pure ()) $ -}do
    ...
    let haskell = ( `fromMaybe` maybeUserSpace ) (instruction "Please ensure that the current working directory is inside the \"haskell\" folder" )
    ...
    addToPATH exeDir
    (elem exeDir <$> getSearchPath) >>= (`unless` instruction ("Please PERMANENTLY add \n\n" ++ exeDir ++ "\n\n to your PATH.\n\nIf you really really don't want to add it to PATH, you can add it temporarily and run again, but please remember to do the same in all future haskell-running terminal sessions and add the path to hlint for your linter."))
